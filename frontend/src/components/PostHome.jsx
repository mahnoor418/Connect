...
const PostHome = ({ post, onPostUpdated, userhe }) => {
    ...
    console.log('PostCard userhe:', userhe);     // already populated
    ...
    useEffect(() => {
  if (author) console.log('📌 author state:', author);
}, [author]);


    useEffect(() => {
        // post.user can be either the full object or just an id string
        const loadAuthor = async () => {
            ...
            try {
                const { data } = await axios.get(
                    `http://127.0.0.1:5000/api/users/${post.user}`
                );
                setAuthor(data);  
                console.log("author : ",data)         // <- expects { _id, username, profilePicture, ... }
            } catch (err) {
                console.error('Could not fetch author data:', err);
            }
        };
    ...
    return (
        <div className="bg-white rounded-xl shadow-lg transition-all hover:shadow-2xl hover:scale-105 transform duration-300 p-6 mb-6 max-w-2xl mx-auto">
            <div className="flex items-center space-x-4 mb-4">
                <img
                    src={`http://127.0.0.1:5000/uploads/${author?.profilePicture}`
                    }
                    alt="Profile"
